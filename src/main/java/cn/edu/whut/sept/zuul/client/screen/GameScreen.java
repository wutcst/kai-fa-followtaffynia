...
public class GameScreen implements Screen
{
    ...
    private void handleUtCombatInput(float delta)
    {
        ...
        if (ut == null) { activeCombatSnapshot = null; return; }
        ...
        if (phase == UndertaleCombatPhase.MENU) {
            if (Gdx.input.isKeyJustPressed(Input.Keys.NUM_1)) ut.selectFight();
                ...
                if (!ut.getDef().actOptions.isEmpty())
                    ut.selectAct(ut.getDef().actOptions.keySet().iterator().next());
            }
            else if (Gdx.input.isKeyJustPressed(Input.Keys.NUM_3)) {
                for (Item it : engine.getPlayer().getInventory()) { ut.selectItem(it.getItemId()); break; }
            }
    ...
    private String formatUtCombat(UndertaleCombatEngine ut)
    {
        ...
        sb.append("[UT] ").append(ut.getDef().displayName)
            ...
            .append("\n").append(ut.getPhaseMessage()).append("\n");
        ...
        if (phase == UndertaleCombatPhase.MENU) {
            ...
            sb.append("WASD=躲避! 子弹:").append(ut.getBullets().size())
                .append(" 灵魂(").append(String.format("%.2f", ut.getSoulX()))
                .append(",").append(String.format("%.2f", ut.getSoulY())).append(")");
        }
        return sb.toString();
    }

    // ========== UT 战斗画中画（最顶层渲染，不干扰 UI） ==========
        ...
        float boxX, float boxY, float boxW, float boxH)
    {
        ...
        for (Bullet b : ut.getBullets()) {
            ...
            if (b.shape == Bullet.Shape.CIRCLE) {
                ...
                shapes.circle(bx, by, Math.max(b.radius * boxW * 0.8f, 3f), 8);
            } else {
                shapes.setColor(1f, 1f, 0.85f, 1f);
                shapes.rect(bx, by, Math.max(b.width * boxW, 4f), Math.max(b.height * boxH, 4f));
            }
        }
        shapes.end();
    }
    ...
    private void drawHpBar(float x, float y, float w, float h, float ratio, boolean enemy)
    {
        shapes.setProjectionMatrix(uiCamera.combined);
        shapes.begin(ShapeRenderer.ShapeType.Filled);
        shapes.setColor(0.12f, 0.12f, 0.12f, 1f);
